...
Five Opus runs of 0.13.1 against #164 and #166 to #169, each refining an item in the 0.12 shape, non-interactive, writes refused by the permission layer so the drafts stayed in the room. All five landed between 445 and 681 words against about 350, and all five gave the same cause: "a line already in the item stays" carried the old spec through, and the repo's `## Shows` said every field verbatim while the template said headline only. Four of five guessed the same hat mapping; five of five copied #165's bold headings. The fix is subtraction, not rules: retention applies only to an item already in the shape, and an item in any other shape is a create from the need it names; Shows is a list of headline field names and nothing else; the template rules over the extend file over the preset, said once; the data rule of 0.12.1 returns, a short table on the story and a long one a story too big, in place of "lives in code", which was false before the code existed and left #168's prefix map with no home. Not added: the countable limits belong in a script, and the seams the five reports named are the graders for an eval. 0.13.2.

## Decided 2026-09-25, two passes

Four Opus runs of 0.13.3 against #164, #166, #168 and #169 landed at 518 to 731 words with no carried-over text left to blame: every draft filled every cap, six examples, three Decided, three Open, because a range reads as a target. Underneath that, the same cause the field run had: the writer reads the check's source at homework, so every criterion is written against the one solution it found, and the story becomes a spec that gets fuzzier rather than a story that many builds could satisfy. `story-create` is story-refine with that removed. No code homework, no decisions step, no sweep, no data table, no identifiers line, no word count, no sentence count; the staleness and derivation tests collapse into one, a line only one way of building could satisfy says how and goes. Each list section holds three, the fourth is the runner's call in one line, a fifth is a split. The builder's pass against the code, which is where decisions, the sweep, the data a rule turns on and the false-premise catch now belong, is a later skill that returns Open lines only. story-refine stays until story-create has runs behind it. 0.14.0.

## Decided 2026-09-25, Northwind

A made-up transcript, six voices on a client whose 19 unbuilt legacy projects dragged the grade, run once through story-create and once through story-refine by Opus with two instructions only: questions go to Open, drafts go to the scratchpad. The two failed in opposite directions. story-create never read code and refused the how, but let names back in as parentheses, kept the run-time noise under Out of scope, turned an unchecked guess into an Example, and grew the opener to seven sentences once the count came off. story-refine read code and caught a false premise, the report words the transcript quoted match nothing the tool prints, but wrote the developer's fix into criterion one as "only when a solution file lists it", placed the Finding in a Category itself, and kept the false premise in its notes because Homework says nothing reaches the story. Both invented a Finding headline, since "the headline verbatim" had no case for a headline that does not exist yet. Seven changes, each a clause on an existing bracket or a deletion: the headline bracket gains its second case; Decided says the thread holds who said it; Out of scope is a case the story could have handled; a question the room did not raise is unassigned; an unchecked claim is an Open line; story-refine gets the one-way test in place of the noun list and the two old tests, and a homework finding that contradicts the seed becomes an Open line; the sweep never demotes a hit to Out of scope to fit. story-create also loses the "why that is wrong" beat from the opener. 0.14.2.

## Decided 2026-09-25, ten Sonnet runs

Five Sonnet runs of story-create and five of story-refine on the Northwind transcript, two instructions only, drafts to the scratchpad. story-create landed 608 to 762 words, story-refine 400 to 504. The unchecked-claim rule held nine of ten; the attribution clause held five of five on story-create and two of five on story-refine, whose homework citation form leaks onto the page. Four of five story-refine runs wrote the solution-file mechanism as criterion one and three placed the Finding in a Category, each calling it settled by the record; the one-way test does not fire in the mind that just read the code, which settles that story-refine goes once the builder's pass exists. story-create leaked the how twice, both times from the developer's line in the transcript rather than from code. No run kept the opener to four sentences once the count was gone, and six of ten parked the run-time aside under Out of scope. Four subtractions and clauses on story-create: the sentence count returns; Out of scope is deleted, a case the story does not handle is an Open line or nothing; an example names nothing the thread did not name, since four runs invented a solution filename; a way of building proposed in the thread is a question, not a rule. 0.14.3.

## Decided 2026-09-25, before showing

Five Sonnet runs of story-create 0.14.3 scored 3 to 4 on the rules and 3 to 4 as stories, 558 to 711 words. Three categories hit 5 on every run, the opener count, unchecked claims to Open, and the noise, and each of those fixes was a number, a deletion, or a routing rule. The spread sat in interpreted sentences: "the thread holds who said it" was read by two runs as leave to cite the thread with timestamps, and "a question the room did not raise" left a raised-but-unowned question to be handed to whoever was nearest. Every two-pass run scored 4 and every one-pass run scored 3. Judged as stories, three details moved value: an Example deciding a question Open still held, an invented owner, and every opener written for the consultant rather than the client whose line the thread already had. 0.14.4 adds a Before-showing step, a script for the countable checks and five yes-or-no questions for the rest, so the second pass is a step rather than a mood; the attribution sentence is deleted in favour of the script; "up to three" replaces "three"; and the three value clauses go on the template brackets in their output form. The prose stopped shrinking variance two rounds ago. The step and the script are where the rest of it goes.

## Decided 2026-09-25, round four

Five Sonnet runs of story-create 0.14.4 scored 3, 4, 4, 4, 4 whole, 526 to 597 words, and check.py printed nothing on every final draft; the pass condition failed on one run. Five categories held at 5 on every run, the opener count, the unchecked claim, the owners, and the two the script now counts, and the two-pass effect is gone: one-pass runs scored 4 and the run with the most fixes scored 3. The regular 2s and 3s share a root. The skill closed the code door, and the thread's own technical talk is the door still open. The how entered as the before-state, the developer's diagnosis of today's code written into two openers as fact and into every count criterion as "not everything on disk"; the routing rule covered proposals only. Decided carried a criterion's why and dragged the what along: the no-config-file line, a rejected way of building, was right five times, while the one-finding-not-nineteen resolution, an outcome and so a criterion, was restated under Decided three times because criteria have no slot for a reason. The third Example slot invented: both runs that wrote three Examples invented a legacy project with tests, which the thread contradicts, runs with two were clean, and Fill still said "three lines" against the template's "up to three"; the run that filled every section to three scored lowest, which backs the open cap-as-target item. 0.14.5 makes five changes and adds no sentence: the Fill count sentence goes; the "one per rule the wording leaves open" clause goes; the opener's fourth beat says "once this lands"; the Decided bracket leads with what was not done; the proposed-fix routing rule also covers how it is built today. Ten runs follow.

## Decided 2026-09-25, story-review

Ten Sonnet runs of story-create 0.14.5 scored 3, 3, 4, 4, 4, 4, 4, 4, 5, 5 on the rule categories, 465 to 568 words, check.py silent on every draft. The generator deletion held: no run wrote a third Example and none invented an edge. Read instead on four columns, value, workable, implementation kept out, and template with the thought leaders behind it, the drafts ranked 10 to 19 of 20. The top two did what the rules could not ask for: one found the edge where the fold rule meets a live project's missing test, the other recorded all three decisions the room made, including two that arrived as pushback rather than announcements. The lowest echoed the opener bracket into its first sentence and wrote the mechanism into every criterion. Every remaining miss has one shape: the head that wrote a line cannot see the how in it, the name in it, or the criterion it restates. Two-pass runs scored higher in every round, and story-refine failed for the same reason from the other side. So the five Before-showing questions leave story-create and become story-review, a skill create runs in a sub-agent that has read neither the draft nor the seed, which edits what the four columns' questions name and returns the draft with one line per change and what it could not fix. story-create gains two routing rules, a proposal the thread turned down is a Decided line, and an assistant in the seed is never an owner or a source of a decision; the criteria bracket gains "nothing named that the thread did not name". check.py loses the per-Name rule, three firings and all false positives, and gains a check for a Decided line that records what the story leaves out. The ten-run gate is retired: one synthetic seed and a spread condition measured the model's wobble, not the story's fit for a client. A three-run smoke on Northwind follows any skill edit; a corpus of client uses, where the runner's edits before the write are the measure, decides the next change. 0.15.0.

## Decided 2026-09-25, first smoke

Three Sonnet runs of 0.15.0 summed 15 to 17 of 20 on the four columns, 477 to 516 words, check.py silent, fifteen review edits across the three and every one traced to a question: names out of openers, owners to unassigned, fields the seed never named cut. One review edit was wrong and structural rather than variance: the reviewer, reading only the seed, changed the report's real headline to the consultant's paraphrase of it, and the writer overrode it. The reviewer has no product words and never will, so it never changes a quoted string; a headline it doubts goes under Could not fix. A second reviewer cut a field from a criterion and left the same words in the opener, so a fix is carried to every line that repeats the words. 0.15.1.

## Decided 2026-09-25, composition

One Sonnet composer read the three smoke drafts and the seed and kept only lines it could trace, then ran story-review; the composite scored 18 of 20 against 17 for the best single draft. Composition is not worth four runs for one point, but its dropped list was. Grounding in the seed kept the one-in-three outlier, Dana's pushback on silent exclusion, and caught two leaks nobody else had: a Decided reason lifted from this repo's AGENTS.md, and an "agent instructions file" the seed never named. It also cut the product's own words, Sub-Score and Punch List, as absent from the seed, the same gap as the headline in the first smoke: a seed-only reader cannot tell product words from invention. Four clauses, 0.15.2: create hands the reviewer the product words it verified and the reviewer never cuts them; a Decided reason is in the thread's words and the reviewer drops one the seed does not hold; a fix is carried to every line that makes the same claim, not the same words, since the narrower rule missed twice in four runs; and a question called someone's because they raised it is not handed to them, which had assigned Lena her own question twice. Findings keys and aggregation across many runs are parked: line indices move on every edit, and matching free-text findings is the fuzzy problem it would create.
