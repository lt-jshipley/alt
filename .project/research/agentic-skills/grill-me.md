...
Of the ten listed above: 1, single skill, no primitive and no passthrough, after two skills were tried and one kept. 2, examine is the one interview and `grill-me` is not a front door to it. 3, rounds, after one-at-a-time was tried in elicit; reversed 2026-09-09 to one question a turn, below. 5, recommendation with its why; no derivation, since no stances exist to derive from. 10, the runner half is rejected rather than undecided: no alt skill models who is running it. The team half stands as the sources file. Decisions 4, 6, 7, 8, and 9 remain open.

### One question a turn, 2026-09-09

Rounds replaced by one question a turn. The wall was cadence times weight: a grilling question is two lines, an examine question ten in the template and fifteen to thirty-five rendered, so a six-question frontier ran well past a screen. Matt Pocock's docs support the opt-out ("When grilling, ask one question at a time" in CLAUDE.md) for people who read slowly, work in a second language, or use the sequence as focus scaffolding; his original text was one at a time, depth-first, finish a branch. The reason examine chose rounds over elicit's walk, a sub-agent chasing a fact while other questions are asked, holds either way: the dependent question waits and another is asked.

Changed: one frontier question per turn; next is a child of the last answer when one is on the frontier, else the costliest if-wrong; the verbs lose their numbers; between questions one line says what settled, retired, and opened, and how many are open, never a total. Kept: the tree and frontier for dependency order, stable numbers, the question shape, the close. Not added: a yes-to-all-remaining verb, because accepting unseen recommendations is the plan-the-agent-wrote failure, and rendering them is the wall again. Gained: an answer can retire a later question before it is shown, and two dependent questions can no longer share a screen. Cost: round trips on a large tree, which grilling's own users accept. Difference from elicit's walk: no first screen, no breadcrumb, no runner hat; the walk was never the reason elicit was replaced.

### The split, 2026-09-22

Story-refine, epic-refine, and brief-create no longer invoke examine. They invoke `alt:decisions`, which carries the method above without the interview and lists roots only. Examine is untouched and remains the interview for a developer working alone. Why, what moved, and what stayed: `decisions.md`.
